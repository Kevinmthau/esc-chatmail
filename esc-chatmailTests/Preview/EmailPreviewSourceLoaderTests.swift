...
final class EmailPreviewSourceLoaderTests: XCTestCase {
    ...
    func testLoadPreviewSourceUsesSharedParsedEmailPathForMigratedFacts() async throws {
        ...
        _ = contentHandler.saveHTML(
            ...
            <html>
            <body>
              <h1>Parsed source title</h1>
              <p>Preview body generated by the shared parsed-email path.</p>
              <img src="https://cdn.example.com/parsed-hero.jpg" width="640" height="320" alt="Hero">
            </body>
            </html>
            """,
            for: messageId
        ...
        let source = await loader.loadPreviewSource(
            ...
            subject: "Parsed source",
            allowRecovery: false
        )

        let unwrappedSource = try XCTUnwrap(source)
        XCTAssertEqual(unwrappedSource.extractedText, "Parsed source title\n\nPreview body generated by the shared parsed-email path.")
        XCTAssertEqual(unwrappedSource.extractedImages.first?.sourceURL, "https://cdn.example.com/parsed-hero.jpg")
        XCTAssertEqual(unwrappedSource.extractedImages.first?.width, 640)
        XCTAssertEqual(unwrappedSource.htmlSummary.h1Text, "Parsed source title")
        let parsedEmailCallCount = await parsedEmailProvider.callCount()
        XCTAssertEqual(parsedEmailCallCount, 1)
        let requestedPreviewImages = await parsedEmailProvider.requestedPreviewImages()
        XCTAssertEqual(requestedPreviewImages, [true])
    }
...
private actor CountingParsedEmailProvider: ParsedEmailProviding {
    private var calls = 0
    private var previewImageRequests: [Bool] = []
    ...
    func parsedEmail(
        ...
        calls += 1
        previewImageRequests.append(includePreviewImages)
    ...
    func requestedPreviewImages() -> [Bool] {
        previewImageRequests
    }
}
